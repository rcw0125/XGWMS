...
function GetCPH()
{
 ...
 //   var aaa=document.getElementById("aaa");
   // aaa.value = drpCPH.options.length;
    if(chkCPH.checked)
    {
        //已经加载过
...
function GetFYDItem()
{
    ...
    if(cheStatus)
    {
        ...
        iFrame.src="FYDItem.aspx?action=item&FYDH="+strFYDH;
    }
    else
    {
        hidFYDHitem.value="";
...
function showkyhw() {
   var grvFYD=document.getElementById("grvFYD");
   var chk=grvFYD.getElementsByTagName("input");
   for(var i=0;i<chk.length;i++)
   {
      if (chk[i].type=="checkbox") {
          if (chk[i].checked) {
              ...
              var inputs=tds[0].all.tags("INPUT");
              var fydh=inputs[1].value;
              var wlh=inputs[5].value;
              var ck=inputs[4].value;
              var sx=inputs[6].value;
              var vfree1=inputs[8].value;
              var vfree2=inputs[9].value;
              var vfree3=inputs[10].value;
              var url="fyd_kyhw.aspx";
              url += "?FYDH=" + encodeURI(fydh);
                    url += "&wlh=" + encodeURI(wlh);
                    url += "&ck="+encodeURI(ck);
                    url += "&sx=" + encodeURI(sx);
                    url += "&vfree1="+encodeURI(vfree1);
                    url += "&vfree2=" + encodeURI(vfree2);
                    url += "&vfree3=" + encodeURI(vfree3);
                    window.open(url,"","toolbar=no,menubar=no,scrollbars=auto, resizable=yes,location=no, status=yes");
          }
      }
   }
   //alert(chkCFYD.parentNode.parentNode.cells(1).innerText);
}

function showFHTJ() {
    var hidFyd = document.getElementById("hidFYDHitem");
    var hidStatus = document.getElementById("hidStatus");
    if (hidFyd.value == "") {
        alert("请选择发运单！");
        return;
    }
    if (hidStatus.value != "1" && hidStatus.value != "0") {
        alert("只能对未装车的进行查询！");
        return;
    }
    var url = "FYD_FHCK.aspx";
    url += "?FYDH=" + encodeURI(hidFyd.value);
    window.open(url, "", "toolbar=no,menubar=no,scrollbars=auto, resizable=yes,location=no, status=yes");
}

function loadkyhwinfo() {

    var hfydh=document.getElementById("hfydh");
    var hck=document.getElementById("hck");
    var hwlh=document.getElementById("hwlh");
    var hsx=document.getElementById("hsx");
    var hvfree1=document.getElementById("hvfree1");
    var hvfree2=document.getElementById("hvfree2");
    var hvfree3=document.getElementById("hvfree3");
    var request=getXmlHttpRequest();
    var url="LogincAjax.aspx?TYPE=10&ck="+hck.value+"&wlh="+hwlh.value+
             "&fydh="+hfydh.value+"&sx="+encodeURI(hsx.value)+"&vfree1="+encodeURI(hvfree1.value)+
             "&vfree2="+encodeURI(hvfree2.value)+"&vfree3="+encodeURI(hvfree3.value);
    sendRequest(url,"","POST",request);
    var result = request.responseText;
    if(result.indexOf("ERROR")!=-1)
    {
    ...
    {
        var grvtbody=document.getElementById("hwinfo");
        var Label1=document.getElementById("Label1");
        grvtbody.innerHTML=result;
